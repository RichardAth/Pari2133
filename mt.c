...
static GEN
mtsingle_queue_get(struct mt_state *mt, int64_t *workid, int64_t *pending)
{
  GEN done = mt->pending;
...
static int64_t single_trace_level = 0;

static void
mtsingle_queue_submit(struct mt_state *mt, int64_t workid, GEN work)
{
  single_is_thread = 1;
...
void
mtsingle_err_recover(int64_t er)
{
  (void) er;
  if (single_is_thread)
  {
...
void
mt_queue_submit(struct pari_mt *pt, int64_t workid, GEN work)
{ pt->submit(&pt->mt, workid, work); }

GEN
mt_queue_get(struct pari_mt *pt, int64_t *workid, int64_t *pending)
