...
sl2_inv(GEN M)
{
  int64_t a=coeff(M,1,1), b=coeff(M,1,2), c=coeff(M,2,1), d=coeff(M,2,2);
  return mkvec2(mkvecsmall2(d, -c), mkvecsmall2(-b, a));
}
/* Return the mat2 [a,b; c,d], not a zm to avoid GP problems */
static GEN
mat2(int64_t a, int64_t b, int64_t c, int64_t d)
...
path_to_ZM(GEN a)
{
  ...
  int64_t r = v[1], s = v[2], x = w[1], y = w[2];
...
static void
p1_std_form(int64_t *pc, int64_t *pd, GEN p1N)
{
...
static int64_t
p1_index(int64_t x, int64_t y, GEN p1N)
{
  ulong N = p1N_get_N(p1N);
...
mfnumcuspsu_fact(GEN fa)
{
  ...
  for (i = 1; i < l; i++)
  {
    int64_t e = E[i], e2 = e >> 1; /* floor(E[i] / 2) */
    ulong p = P[i];
    if (odd(e))
      T *= 2 * upowuu(p, e2);
    else
...
mfnumcusps_fact(GEN fa)
{
  ...
  {
    GEN p = gel(P,i), c;
    int64_t e = itos(gel(E,i)), e2 = e >> 1; /* floor(E[i] / 2) */
    if (odd(e))
...
inithashcusps(GEN p1N)
{
  ...
  int64_t k, ind, l = lg(div), ncusp = mfnumcuspsu_fact(p1N_get_fa(p1N));
...
{
  int64_t p, N = gel(S,1)[1], q = umodsu(c[2], N);
  ulong u, d;
...
GEN
Qevproj_apply_vecei(GEN T, GEN pro, int64_t k)
{
...
static GEN
ZX_factor_limit(GEN T, int64_t deglim, int64_t *pl)
{
 ...
 * Eg for H = msnew */
static GEN
mssplit_i(GEN W, GEN H, int64_t deglim)
{
  ...
  while ((p = u_forprime_next(&S)))
  {
    ...
    for (j = first; j < lV; j++)
    {
      ...
      { /* can split Vj */
        ...
        for (k = 1; k < lF; k++)
        {
          int64_t d = degpol(gel(F,k));
          if (d > D) D = d;
        }
        /* remove V[j] */
        gel(V,j) = gel(V,lg(V)-1); setlg(V, lg(V)-1);
        pows = RgM_powers(TVj, minss((int64_t)2*sqrt((double)D), D));
        for (k = 1; k < lF; k++)
        {
          GEN f = gel(F,k);
...
GEN
mssplit(GEN W, GEN H, int64_t deglim)
{
  pari_sp av = avma;
  checkms(W);
  if (!msk_get_sign(W))
...
msqexpansion_i(GEN W, GEN proV, ulong B)
{
  ...
  int64_t i, d, k = msk_get_weight(W);
  forprime_t S;
...
{
  int64_t w,z, d = cbezout(p, -(int64_t)M, &w, &z);
  if (d != 1) return NULL;
  return mat2(w,z,M,p);
}
static GEN
WQ_matrix(int64_t N, int64_t Q)
{
  int64_t w,z, d = cbezout(Q, N/Q, &w, &z);
  if (d != 1) return NULL;
...
msnew(GEN W)
{
  ...
  ulong N = ms_get_N(W);
  int64_t s = msk_get_sign(W), k = msk_get_weight(W);
  ...
  for (;;)
  {
    ...
    for (c = C; c; c = c->next)
    {
      ...
      { /* YES */
        ...
        {
          int64_t a1 = coeff(gam, 1,1), a2 = coeff(gam, 1,2);
          int64_t a = a1 + a2; /* gcd(a,b) = 1 */
...
static GEN
Gamma0N_decompose(GEN W, GEN M, int64_t *index)
{
  ...
  int64_t s, ind = p1_index(c, d, p1N); /* as an elt of P1(Z/NZ) */
...
path_Gamma0N_decompose(GEN W, GEN path)
{
  ...
  int64_t p1index = p1_index(cc(M), dd(M), p1N);
  int64_t ind = p1index_to_ind[p1index];
...
form_E_F_T(ulong N, GEN p1N, GEN *pC, PS_sets_t *S)
{
  ...
  int64_t nbgen = lg(cusp_list)-1, nbmanin = p1_size(p1N), r, s, i;
  hashtable *F, *T2, *T31, *T32, *E1, *E2;
...
paths_decompose(GEN W, hashtable *h, int flag)
{
  ...
  for (i = 1; i < l; i++)
  {
    ...
    int64_t index = p1_index(cc(M), dd(M), p1N);
...
static GEN
reverse_list(GEN x, int64_t max)
{
 ...
 * = b - a paths */
static GEN
F_indices(GEN W, int64_t a, int64_t b)
{
...
static GEN
F_indices_oo(GEN W, int64_t end, int64_t a, int64_t b)
{
...
msinit_N(ulong N)
{
  ...
  * W2[j] associates to the j-th element of this list its index in P1. */
  ...
  for (r = 1; r <= F->nb; r++)
  {
    ...
    int64_t a = w[1], b = w[2], d = labs(b - a);
...
mspathgens_i(GEN W)
{
  ...
  else
  {
    ...
    int64_t j, nbT2 = lg(annT2)-1, nbT31 = lg(annT31)-1, nbE1 = ms_get_nbE1(W);
...
static int64_t
set_from_index(GEN W11, int64_t i)
{
  if (i <= W11[1]) return 1;
  if (i <= W11[2]) return 2;
  if (i <= W11[3]) return 3;
...
/* det M = 1 */
static void
treat_index(GEN W, GEN M, int64_t index, GEN v)
{
  GEN W11 = gel(W,11);
  int64_t shift = W11[3]; /* #F + #E2 + T32 */
  switch(set_from_index(W11, index))
  {
    ...
    {
      int64_t T3shift = W11[5] - W11[2]; /* #T32 + #E1 + #T2 */
      index += T3shift;
      index -= shift;
...
static void
treat_index_trivial(GEN v, GEN W, int64_t index)
{
  GEN W11 = gel(W,11);
  int64_t shift = W11[3]; /* #F + #E2 + T32 */
  switch(set_from_index(W11, index))
  {
    ...
    {
      int64_t r = index - W11[1];
      int64_t s = E2fromE1_c(gel(msN_get_E2fromE1(W), r));
      v[s]--;
      break;
    }
...
/* [a,b;c,d] * cusp */
static GEN
cusp_mul(int64_t a, int64_t b, int64_t c, int64_t d, GEN cusp)
{
  int64_t x = cusp[1], y = cusp[2];
  int64_t A = a*x+b*y, B = c*x+d*y, u = cgcd(A,B);
  if (u != 1) { A /= u; B /= u; }
  return mkcol2s(A, B);
}
...
{
  int64_t a = coeff(f,1,1), b = coeff(f,1,2);
  int64_t c = coeff(f,2,1), d = coeff(f,2,2);
...
static hashtable *
Gl2act_cache(int64_t dim) { return set_init(dim*10); }
...
init_dual_act(GEN v, GEN W1, GEN W2, struct m_act *S)
{
  ...
  int64_t j, lv = lg(v), dim = S->dim == 1? ms_get_nbE1(W2): lg(gen)-1;
...
dense_act_col(GEN col, GEN phi)
{
  ...
  int64_t i, l = lg(colind), lphi = lg(phi);
  for (i = 1; i < l; i++)
  {
    int64_t a = colind[i];
    GEN t;
...
sparse_act_col(GEN col, GEN phi)
{
  ...
  for (a = 1; a < l; a++)
  {
    ...
    int64_t i = zv_search(colind, ind[a]);
    if (!i) continue;
    t = RgM_RgC_mul(gel(colval,i), t);
    s = s? RgC_add(s, t): t;
  }
 ...
 * see init_dual_act. */
static GEN
dual_act(int64_t dimV, GEN act, GEN phi)
{
  int64_t l = lg(act), j;
  GEN v = cgetg(l, t_MAT);
...
/* in level N > 1 */
static void
msk_get_st(GEN W, int64_t *s, int64_t *t)
...
getMorphism_basis(GEN W, GEN phi)
{
  ...
  for (i = 2; i < lg(link); i++)
  {
    ...
    for (j =1 ; j < lg(L); j++)
    {
      int64_t n = L[j]; /* phi_{i,j} = basis[n] */
      GEN mu_ij = gel(R, n);
...
static GEN
ZGl2Q_act_s(GEN b, GEN a, int64_t k)
{
  if (typ(b) == t_INT)
  {
...
getMorphism(GEN W1, GEN W2, GEN v)
{
  struct m_act S;
  GEN B1, M, act;
  int64_t a, l, k = msk_get_weight(W1);
...
static GEN
msatkinlehner_i(GEN W, int64_t Q)
{
  int64_t N = ms_get_N(W);
  GEN v;
...
GEN
msatkinlehner(GEN W, int64_t Q, GEN H)
{
  pari_sp av = avma;
  GEN w;
  int64_t k;
...
/* return E_c(r) */
static GEN
get_Ec_r(GEN c, int64_t k)
{
  int64_t p = c[1], q = c[2], u, v;
  GEN gr;
...
msfromcusp_trivial(GEN W, GEN c)
{
  ...
  int64_t j, ic = cusp_index(c, S), l = ms_get_nbE1(W)+1;
  GEN phi = cgetg(l, t_COL);
  for (j = 1; j < l; j++)
  {
    GEN vj, g = gel(section, gen[j]); /* path_to_zm(generator) */
    GEN c1 = gel(g,1), c2 = gel(g,2);
    int64_t i1 = cusp_index(c1, S);
    int64_t i2 = cusp_index(c2, S);
    if (i1 == ic)
...
msfromcusp_i(GEN W, GEN c)
{
  ...
  int64_t j, ic, l, k = msk_get_weight(W);
  if (k == 2)
  {
    int64_t N = ms_get_N(W);
...
#if 0
static int64_t
TpS_char_bound(ulong p, int64_t k, int64_t d)
{ /* |eigenvalue| <= 2 p^(k-1)/2 */
  return d * (2 + (log2((double)p)*(k-1))/2);
}
#endif
static int64_t
TpE_char_bound(ulong p, int64_t k, int64_t d)
{ /* |eigenvalue| <= 2 p^(k-1) */
...
static int
use_Petersson(int64_t N, int64_t k, int64_t s)
{
  if (!s)
  {
    if (N == 1)  return 1;
...
static GEN
RgMV_find_non_zero_last_row(int64_t offset, GEN V)
{
  int64_t i, lasti = 0, lastj = 0, lV = lg(V);
  GEN m = NULL;
  for (i = 1; i < lV; i++)
  {
    GEN M = gel(V,i);
...
{
  int64_t l = lg(P); *pd = gel(P, l-1);
  P = vecslice(P, 1, l-2);
  return shallowconcat(gen_0, ZC_apply_dinv(dinv, P));
}

static GEN
get_phi_ij(int64_t i,int64_t j,int64_t n, int64_t s,int64_t t,GEN P_st,GEN Q_st,GEN d_st,
...
/* WN = msinit_N(N) */
static GEN
mskinit(ulong N, int64_t k, int64_t sign)
{
  GEN W, WN = msinit_N(N);
  if (N == 1)
  {
...
GEN
msinit(GEN N, GEN K, int64_t sign)
{
  pari_sp av = avma;
  GEN W;
  int64_t k;
...
static GEN
eval_single(GEN s, int64_t k, GEN B, int64_t v)
{
  int64_t i, l;
...
static GEN
mseval_by_values(GEN W, GEN s, GEN p, int64_t v)
{
  int64_t i, l, k = msk_get_weight(W);
  GEN A;
  if (k == 2)
...
static GEN
allxpm(GEN W, GEN xpm, int64_t f)
{
...
static GEN
ell_get_Cw(GEN LE, GEN W, GEN xpm, int64_t s)
{
  int64_t f, NE = ms_get_N(W);
  const int64_t bit = 64;
...
static GEN
ell_get_scale(GEN LE, GEN W, int64_t sign, GEN x)
{
  if (sign)
...
/* E minimal */
static GEN
msfromell_scale(GEN x, GEN Cw, GEN E, int64_t s)
{
  GEN B = int2n(32);
  if (s)
  {
...
static void
msfromell_l(GEN *pxl, GEN K, GEN star, int64_t sign, ulong l)
{
...
static int
msfromell_check(GEN x, GEN vT, GEN star, int64_t sign)
{
  int64_t i, l;
  GEN sx;
  if (!x) return 0;
...
static GEN
init_moments_act(GEN W, int64_t p, int64_t n, GEN q, GEN v)
{
  struct m_act S;
  int64_t k = msk_get_weight(W);
  S.p = p;
  S.k = k;
  S.q = q;
  S.dim = n+k-1;
...
static void
clean_tail(GEN phi, int64_t c, GEN q)
{
  int64_t a, l = lg(phi);
  for (a = 1; a < l; a++)
  {
...
static GEN
red_mod_FilM(GEN phi, ulong p, int64_t k, int64_t flag)
{
  int64_t a, l;
...
{
  int64_t t, i, k = mspadic_get_weight(W);
  int64_t p = mspadic_get_p(W), n = mspadic_get_n(W);
...
static GEN
oms_dim1(GEN W, GEN phi, GEN alpha, int64_t flag)
{
  int64_t i, k = mspadic_get_weight(W);
  int64_t p = mspadic_get_p(W), n = mspadic_get_n(W);
...
GEN
mspadicinit(GEN W, int64_t p, int64_t n, int64_t flag)
{
  pari_sp av = avma;
  int64_t a, N, k;
...
GEN
mspadic_unit_eigenvalue(GEN ap, int64_t k, GEN p, int64_t n)
{
...
mstooms(GEN W, GEN phi)
{
  ...
  { /* p-stabilize */
    ...
    { /* p | ap, alpha = [a_p, -1; p^(k-1), 0] */
      int64_t flag = mspadic_get_flag(W);
...
GEN
mspadicmoments(GEN W, GEN PHI, int64_t D)
{
  pari_sp av = avma;
  int64_t na, ia, b, lphi, aD = labs(D), pp, p, k, n, vden;
...
static GEN
xlog1x(int64_t n, int64_t zk, int64_t logj, int64_t *pteich)
{
...
static GEN
mspadicint(GEN oms, int64_t teichi, GEN S)
{
  pari_sp av = avma;
  int64_t p = oms_get_p(oms), n = oms_get_n(oms), n0 = oms_get_n0(oms);
  GEN vT = gel(oms,1), alpha = gel(oms,2), gp = utoipos(p);
  int64_t loss = S? Z_lval(Q_denom(S), p): 0;
  int64_t nfinal = minss(n-loss, n0);
  int64_t i, la, l = lg(gel(vT,1));
...
GEN
mspadicseries(GEN oms, int64_t teichi)
{
  pari_sp av = avma;
  GEN S, L, X, vlog, s, s2, u, logu, bin;
  int64_t j, p, m, n, step, stop;
  checkoms(oms);
  n = oms_get_n0(oms);
  if (n < 1)
  {
 ...
 */
GEN
mspadicL(GEN oms, GEN s, int64_t r)
{
  pari_sp av = avma;
  GEN s1, s2, z, S;
  int64_t p, n, teich;
...
struct siegel
{
  GEN V, Ast;
  int64_t N; /* level */
  int64_t oo; /* index of the [oo,0] path */
  int64_t k1, k2; /* two distinguished indices */
  int64_t n; /* #W, W = initial segment [in siegelstepC] already normalized */
};
...
siegel_init(struct siegel *C, GEN M)
{
  ...
  int64_t i, l = lg(L), n = lg(E1T)-1, lE = lg(E2);

  Ast = cgetg(l, t_VECSMALL);
  for (i = 1; i < lE; ++i)
  {
    int64_t j = E2fromE1_c(gel(E2fromE1,i));
    Ast[n+i] = j;
    Ast[j] = n+i;
  }
  for (; i<=n; ++i) Ast[i] = i;
  ...
  {
    GEN c = gel(S, L[i]);
    int64_t c12, c22, c21 = ucoeff(c,2,1);
...
static GEN
rotate_perm(int64_t l, int64_t a)
{
...
/* a1 < c1 <= a2 < c2*/
static GEN
basic_op_perm(int64_t l, int64_t a1, int64_t a2, int64_t c1, int64_t c2)
{
  GEN p = cgetg(l, t_VECSMALL);
  int64_t i, j = 1;
  p[a1] = j++;
...
static GEN
basic_op_perm_elliptic(int64_t l, int64_t a1)
{
  GEN p = cgetg(l, t_VECSMALL);
  int64_t i, j = 1;
  p[a1] = j++;
 ...
 * V[x..y-1] <- g*V[x..y-1] */
static void
path_vec_mul(GEN V, int64_t x, int64_t y, GEN g)
{
  int64_t j;
  GEN M;
  if (x == y) return;
...
static int64_t prev(GEN V, int64_t i) { return (i == 1)? lg(V)-1: i-1; }
static int64_t next(GEN V, int64_t i) { return (i == lg(V)-1)? 1: i+1; }
static GEN
ZM_det2(GEN u, GEN v)
{
...
{
  pari_sp av = avma;
  int64_t s = signe(ZM_det2(u, v));
  return gc_long(av, s);
}
static void
fill1(GEN V, int64_t a)
{
  int64_t p = prev(V,a), n = next(V,a);
...
/* a1 < a2 */
static void
fill2(GEN V, int64_t a1, int64_t a2)
{
  if (a2 != a1+1) { fill1(V,a1); fill1(V,a2); } /* non adjacent, reconnect */
  else
  { /* parabolic */
    int64_t p = prev(V,a1), n = next(V,a2);
...
static GEN
ZM2_div(GEN T, GEN U, GEN DU, int64_t N)
{
...
static GEN
get_g(struct siegel *S, int64_t a1)
{
  pari_sp av = avma;
  int64_t a2 = S->Ast[a1];
...
static void
basic_op(struct siegel *S, int64_t a1, int64_t c1, int64_t c2)
{
  pari_sp av;
  int64_t l = lg(S->V), a2 = S->Ast[a1];
  GEN g;
...
/* a = a^* (elliptic case) */
static void
basic_op_elliptic(struct siegel *S, int64_t a1)
{
  pari_sp av;
  int64_t l = lg(S->V);
  GEN g = get_g(S, a1);
...
decorate(pari_str *s, GEN g, GEN arc, double high)
{
  ...
  if (a + d)
  {
    ...
    int64_t D1 = itos(gcoeff(arc,2,1));
    int64_t D2 = itos(gcoeff(arc,2,2));
...
#if 0
static int
iselliptic(GEN Ast, int64_t i) { return i == Ast[i]; }
static int
isparabolic(GEN Ast, int64_t i)
{ int64_t i2 = Ast[i]; return (i2 == i+1 || i2 == i-1); }
#endif
...
static GEN
binomial_init(int64_t n, GEN vC)
{
...
actf(int64_t N, GEN M, GEN g)
{
  ...
  for (n = 1; n < l; n++)
  {
    GEN v = gel(M,n);
    int64_t i = v[1], j = v[2];
    int64_t I = Fl_add(Fl_mul(a,i,N), Fl_mul(c,j,N), N);
    int64_t J = Fl_add(Fl_mul(b,i,N), Fl_mul(d,j,N), N);
    if (!I) I = N;
    if (!J) J = N;
...
static GEN
eisf(int64_t N, int64_t C, int64_t a, int64_t d1, GEN Z2, int64_t c1, int64_t c2,
     int64_t q1, int64_t u)
{
  GEN m = cgetg(C+1, t_VEC);
  int64_t v, n = 1, l = lg(Z2);
  for (v = 1; v <= a; v++)
    if (ugcd(v,a)==1)
    {
      int64_t w1 = Fl_div(u, v, a), vq1 = v * q1, i, j;
      for (i = 0; i < d1; i++, w1 += a)
      { /* w1 defined mod a*d1, lifts u/v (mod a) */
        for (j = 1; j < l; j++)
          if (Z2[j])
          {
            int64_t wq2 = (c1 * w1 + c2 * j) % N;
            if (wq2 <= 0) wq2 += N;
...
static GEN
eisspace(int64_t N, int64_t k, int64_t s)
{
  GEN v, D, F = factoru(N);
  int64_t l, n, i, j;
  ...
  {
    int64_t d = D[i], Nd = D[l-i], a = ugcd(d, Nd), q1, q2, d1, d2, C, c1, c2, u;
    GEN Z2;
...
{
  int64_t n, t, l = lg(fg), N = co_get_N(co), k = co_get_k(co);
  GEN P, B, z, T;
  pari_sp av;
...
{
  int64_t j, n, k = co_get_k(co), l = lg(fg);
...
evalhull(GEN co, GEN f, GEN r)
{
  ...
  int64_t j, l = lg(V), N = co_get_N(co);
  for (j = 2; j < l; j++)
  {
...
/*vB[j][i] = {i/N} */
static GEN
get_bern(int64_t N, int64_t k)
{
  GEN vB = cgetg(k+1, t_VEC), gN = utoipos(N);
  int64_t i, j; /* no need for j = 0 */
  for (j = 1; j <= k; j++)
  {
...
{
  pari_sp av = avma;
  int64_t j, n = msdim(M), s = msk_get_sign(M);
...
eisker(GEN M)
{
  int64_t N = ms_get_N(M), k = msk_get_weight(M), s = msk_get_sign(M);
  GEN worker, vC, co, CD, D, B, BD, T, E = eisspace(N, k, s);
  int64_t i, j, m = lg(E)-1, n = msdim(M), pending = 0;
  struct pari_mt pt;
