...
{
  int64_t s = signe(z);
  return s==0 ? 0: (int64_t)(s*mod2BIL(z));
}
...
static int64_t
possible(GEN F, GEN Ftr, GEN V, GEN W, GEN per, int64_t I, int64_t J)
{
  int64_t i, j, k, count = 0, n = lg(W)-1, f = lg(F)-1;
...
fingerprint(struct fingerprint *fp, struct qfauto *qf)
{
  ...
  int64_t i, j, k, min, dim = qf->dim, n = lg(V)-1, f = lg(F)-1;
...
static GEN
bacher(int64_t I, int64_t S, struct qfauto *qf)
{
  pari_sp av = avma;
  GEN V=qf->V, W=qf->W, Fv=gel(qf->v,1), list, listxy, counts, vI, coef;
  int64_t i, j, k, nlist, nxy, sum, mind, maxd, n = lg(V)-1;
  ...
  for (i = 1; i <= n; ++i)
    ...
    {
      int64_t s = zv_dotproduct(vI, gel(Fv,i));
      if (s == S) list[++nlist] = i;
      if (-s == S) list[++nlist] = -i;
    }
  ...
  for (i = 1; i <= nlist; ++i)
  {
    ...
    for (j = 1; j <= nxy; ++j)
    {
      int64_t S1 = listxy[j] > 0 ? S : -S;
      GEN Vj = gel(V, labs(listxy[j]));
      for (k = j+1; k <= nxy; ++k)
      {
        int64_t S2 = listxy[k] > 0 ? S1 : -S1, lk = labs(listxy[k]);
...
static GEN
init_bacher(int64_t bachdep, struct fingerprint *fp, struct qfauto *qf)
{
  GEN z = cgetg(bachdep+1,t_VEC);
  int64_t i;
  for (i=1;i<=bachdep;i++)
  {
    int64_t bachscp = mael(qf->W,fp->e[i],1) / 2;
...
static int64_t
bachcomp(GEN pol, int64_t I, GEN V, GEN W, GEN Fv)
{
  pari_sp av = avma;
  GEN co, list, listxy, vI, coef = gel(pol,2);
  int64_t i, j, k, nlist, nxy, count;
  const int64_t n = lg(V)-1, S = mael(W,I,1) / 2;
  int64_t sum = mael(pol,1,1), mind = mael(pol,1,2), maxd = mael(pol,1,3);
  vI = gel(V,I);
  list = zero_Flv(sum);
...
{
  int64_t i, j, k, n = lg(V)-1, f = lg(F)-1;
...
static int64_t
operate(int64_t nr, GEN A, GEN V)
{
  pari_sp av = avma;
  int64_t im,eps;
...
static GEN
orbit(GEN pt, int64_t ipt, int64_t npt, GEN H, GEN V)
{
  pari_sp av = avma;
  int64_t i, cnd, im, n = lg(V)-1, nH = lg(H)-1, no = npt;
...
static int64_t
orbitlen(int64_t pt, int64_t orblen, GEN G, int64_t nG, GEN V)
{
  pari_sp av = avma;
  int64_t i, len, cnd, n = lg(V)-1;
  GEN orb, flag;
  ...
  for (cnd = 1; cnd <= len && len < orblen; cnd++)
    ...
    {
      int64_t im = operate(orb[cnd], gel(G,i), V);
      /* image is a new point in the orbit */
...
{
  int64_t i, j, len, l1 = lg(orb1)-1, l2 = lg(orb2)-1;
...
static int64_t
orbsubtract(GEN Cs, GEN pt, int64_t ipt, int64_t npt, GEN H, GEN V, int64_t *len)
{
  pari_sp av = avma;
 ...
 * <G->g[i],...,G->g[n-1]> */
static void
stab(int64_t I, struct group *G, struct fingerprint *fp, GEN V, ulong p)
{
  GEN orb, w, flag, H, Hj, S;
  int64_t len, cnd, i, j, k, l, im, nH, fail;
  int64_t Maxfail, Rest, dim = lg(fp->diag)-1, n = lg(V)-1;
  ...
  while (cnd <= len && fail < Maxfail+Rest)
  {
    for (i = 1; i <= nH && fail < Maxfail+Rest; ++i)
    {
      ...
      {
        cnd = 1+(int64_t)random_Fl(len);
        i = 1+(int64_t)random_Fl(nH);
      }
      im = operate(orb[cnd], gel(H,i), V);
 ...
 * (should be fp.diag[I]) */
static int64_t
qfisom_candidates_novec(GEN CI, int64_t I, GEN x, struct qfauto *qf,
       struct qfauto *qff, struct fingerprint *fp)
{
  pari_sp av = avma;
  int64_t i, j, k, okp, okm, nr, fail;
  GEN vec, F =qf->F, V=qff->V, W=qff->W, v=qff->v;
  int64_t n = lg(V)-1, f = lg(F)-1;
...
static int64_t
qfisom_candidates(GEN CI, int64_t I, GEN x, struct qfauto *qf,
      struct qfauto *qff, struct fingerprint *fp, struct qfcand *qfcand)
{
  ...
  int64_t i, j, k, okp, okm, nr, nc, vj, rank, num;
  int64_t dim = qf->dim, n = lg(V)-1, f = lg(F)-1;
  int64_t DEP = qfcand->cdep, len = f * DEP;
  if (I >= 2 && I <= lg(qfcand->bacher_pol))
  {
    int64_t t = labs(x[I-1]);
...
static int64_t
aut(int64_t step, GEN x, GEN C, struct group *G, struct qfauto *qf,
...
autom(struct group *G, struct qfauto *qf, struct fingerprint *fp,
      struct qfcand *cand)
{
  int64_t i, j, step, im, nC, found, tries, nbad;
  GEN x, bad, H, V = qf->V;
  int64_t dim = qf->dim, n = lg(V)-1, STAB = 1;
...
zm_maxdiag(GEN A)
{
  int64_t dim = lg(A)-1, max = coeff(A,1,1), i;
  for (i = 2; i <= dim; ++i)
...
static GEN
init_qfauto(GEN F, GEN U, int64_t max, struct qfauto *qf, GEN norm, GEN minvec)
{
  GEN V = minvec ? ZM_to_zm_canon(minvec)
                 : gel(minim_zm(zm_to_ZM(gel(F,1)), stoi(max), NULL), 3);
  GEN W, v;
  int64_t i, j, k, n = lg(V)-1, f = lg(F)-1, dim = lg(gel(F,1))-1;
  for (i = 1; i <= n; i++)
  {
    GEN Vi = gel(V,i);
    for (k = 1; k <= dim; k++)
 ...
 * them on scpvec  */
static GEN
scpvector(GEN w, GEN b, int64_t I, int64_t dep, GEN v)
{
  int64_t  i, j, n = lg(v)-1;
...
static GEN
scpvecs(GEN *pt_vec, int64_t I, GEN b, int64_t dep, struct qfauto *qf)
{
  GEN list, vec, V = qf->V, F = qf->F, v = qf->v;
  int64_t n = lg(V)-1, dim = lg(gel(F,1))-1, len = (lg(F)-1)*dep;
  int64_t j;
...
{
  GEN F = qf->F;
  int64_t i, j, k, n = lg(F)-1, dim = lg(gel(F,1))-1, nb = lg(b)-1;
...
static GEN
gen_comb(int64_t cdep, GEN A, GEN e, struct qfauto *qf, int64_t lim)
{
  int64_t i, dim = lg(A)-1;
...
static GEN
qfisom_bestmat(GEN A, int64_t *pt_max)
{
  int64_t max = zm_maxdiag(A), max2;
...
init_qfisom(GEN F, struct fingerprint *fp, struct qfcand *cand,
            struct qfauto *qf, GEN flags, int64_t *max, GEN minvec)
{
  GEN U, A, norm;
  if (is_qfisom(F))
  {
 ...
 * generators in H */
static GEN
isostab(int64_t pt, GEN G, GEN V, int64_t Maxfail, ulong p)
{
  pari_sp av = avma;
  int64_t  i, im, nH, fail, len, cnd, orblen, rpt;
  int64_t dim = lg(gel(V,1))-1, n = lg(V)-1, nG = lg(G)-1;
  GEN w, flag, orb, H;
...
static int64_t
iso(int64_t step, GEN x, GEN C, struct qfauto *qf, struct qfauto *qff,
...
{
  pari_sp av = avma;
  int64_t i, j, c, rows = nbrows(M), cols = lg(M)-1;
...
GEN
qfautoexport(GEN G, int64_t flag)
{
  pari_sp av = avma;
  int64_t i, lgen,  c = 2;
  ...
  {
    int64_t dim = lg(gmael(gen,1,1))-1;
