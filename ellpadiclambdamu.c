...
static GEN
initQplog(int64_t p, ulong q, int64_t n)
{
  int64_t i, nn, nt;
  ulong a, P;
  GEN C;
 ...
 * (-1)^i (D/-1) - part */
static GEN
polPn(GEN W, GEN xpm, int64_t p, int64_t D, int64_t R, int64_t n)
{
  pari_sp av = avma, av2;
  int64_t N = (p == 2)? n + 2: n + 1;
...
static int64_t
lambda_ss(GEN W, GEN xpm, int64_t v, int64_t p, int64_t D, int64_t R, int64_t n)
{
  for (;; n += 2)
  {
...
/* return lambda, assuming mu = 0 [else infinite loop] */
static int64_t
lambda_ord(GEN W, GEN xpm, int64_t v, int64_t p, int64_t D, int64_t R, GEN ap)
{
  GEN P1, P0 = polPn(W, xpm, p, D, R, 0);
  int64_t n;
  for (n = 1;; n++, P0 = P1)
  {
    int64_t mu, pn = upowuu(p,n);
...
GEN
ellpadiclambdamu(GEN E, int64_t p, int64_t D, int64_t R)
{
  pari_sp av = avma;
  int64_t vC, s, muadd = 0;
  GEN W, xpm, C, ap;
  ...
  if (ap)
  { /* ordinary */
    int64_t L = lambda_ord(W, xpm, vC, p, D, R, ap);
    set_avma(av); return mkvec2s(L, muadd);
  }
  else
  {
    int64_t Lp = lambda_ss(W, xpm, vC, p, D, R, 0);
    int64_t Lm = lambda_ss(W, xpm, vC, p, D, R, 1);
