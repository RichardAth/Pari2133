...
Qp_agm2_sequence(GEN a1, GEN b1)
{
  ...
  int64_t pp = precp(a1), v = valp(a1), i;
...
Qp_ascending_Landen(GEN AB, GEN *ptx, GEN *pty)
{
  ...
  int64_t n = lg(R)-1, va = itos(gel(AB,4)), v, i;
...
static GEN
doellQp_root(GEN E, int64_t prec)
{
  ...
  { /* Use 216T(X/3) = 32X^3 - 6c4 X - c6 to have integral root; a=-c6 mod 3 */
    ...
    switch(lg(a)-1)
    {
      ...
      case 3: /* three roots, e.g. "15a1", choose the right one */
      {
        ...
        int64_t v1 = Z_lval(subii(a2, a3), 3);
        int64_t v2 = Z_lval(subii(a1, a3), 3);
        int64_t v3 = Z_lval(subii(a1, a2), 3);
        if      (v1 == v2) a = a3;
        else if (v1 == v3) a = a2;
        else a = a1;
      }
...
/* t to w := -1/y */
GEN
ellformalw(GEN e, int64_t n, int64_t v)
{
  pari_sp av = avma, av2;
...
/* t to [x,y] */
GEN
ellformalpoint(GEN e, int64_t n, int64_t v)
{
  pari_sp av = avma;
...
GEN
ellformaldifferential(GEN e, int64_t n, int64_t v)
{
  pari_sp av = avma;
...
/* t to z, dz = w1 dt */
GEN
ellformallog(GEN e, int64_t n, int64_t v)
{
  pari_sp av = avma;
  GEN w = ellformalw(e, n, v), wi = ser_inv(w), x;
  GEN w1 = ellformaldifferential_i(e, w, wi, &x);
...
/* z to t */
GEN
ellformalexp(GEN e, int64_t n, int64_t v)
{
  pari_sp av = avma;
...
static GEN
ellformallogsigma_t(GEN e, int64_t n)
{
  pari_sp av = avma;
 ...
 * return t = -x/y + O(p^v) */
static GEN
tfromx(GEN e, GEN x, GEN p, int64_t v, GEN N, GEN *pd)
{
...
static int64_t
logsigma_prec(GEN p, int64_t v, int64_t t)
{
  double log2p = dbllog2(p);
  int64_t j, i = ceil((v - t) / (t - 2*M_LN2/(3*log2p)) + 0.01);
...
static int64_t
log_prec(GEN p, int64_t v, int64_t t)
{
  double log2p = dbllog2(p);
  int64_t j, i = ceil(v / (t - M_LN2/(2*log2p)) + 0.01);
  /* guaranteed to work, now optimize */
  for (j = i-1; j >= 1; j--)
  {
...
ellnonsingularmultiple(GEN e, GEN P)
{
  ...
  for (i = 1; i < l; i++)
  {
    ...
    {
      int64_t nu = kod - 4;
      int64_t n = minss(Q_pval(ec_dmFdy_evalQ(E,P), p), nu/2);
      nu /= ugcd(nu, n);
      g = muliu(g, nu);
...
GEN
ellpadicheight(GEN e, GEN p, int64_t v0, GEN P)
{
  pari_sp av = avma;
...
GEN
ellpadiclog(GEN E, GEN p, int64_t n, GEN P)
{
  pari_sp av = avma;
  int64_t vt;
  GEN t, x, y, L;
...
static GEN
ellpadics2_tate(GEN Ep, int64_t n)
{
  pari_sp av;
...
GEN
ellpadicfrobenius(GEN E, ulong p, int64_t n)
{
  checkell(E);
...
/* s2 = (b_2-E_2)/12 */
GEN
ellpadics2(GEN E, GEN p, int64_t n)
{
  pari_sp av = avma;
  GEN l, F, a,b,d, ap;
  ulong pp;
...
static GEN
ellpadicL_init(GEN W, int64_t n)
{
  GEN Wp, den, M = gel(W,1), xpm = gel(W,2), ap = gel(W,3), s = gel(W,5);
  int64_t p = itos(gel(W,4)), D = itos(gel(W,6));
...
static GEN
ellpadic_i(GEN v, int64_t r)
{
  GEN oms = gel(v,1), den = gel(v,2), s = gel(v,3);
  return gdiv(mspadicL(oms,s,r), den);
}
GEN
ellpadicL(GEN E, GEN p, int64_t n, GEN s, int64_t r, GEN D)
{
  pari_sp av = avma;
  GEN W, v;
...
GEN
ellpadicbsd(GEN E, GEN p, int64_t n, GEN D)
{
  const int64_t MAXR = 30;
  pari_sp av = avma;
...
GEN
ellpadicregulator(GEN E, GEN p, int64_t n, GEN S)
{
  pari_sp av = avma;
