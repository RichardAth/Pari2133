...
static const int64_t VERYBIG = (1LL<<20);
static int64_t
...
/* min(a,b,c) */
static int64_t
min3(int64_t a, int64_t b, int64_t c)
{
  int64_t m = a;
  if (b < m) m = b;
  if (c < m) m = c;
...
static GEN
padicfactors(GEN pol, GEN p, int64_t r) { return gel(factorpadic(pol,p,r),1); }
...
static int64_t
discpart(GEN H, GEN p, int64_t prec)
{
  GEN list, prod, dis;
  int64_t i, j;
...
static int64_t
theta_j(GEN B, GEN p, int64_t j)
{
  int64_t i, t = 60*myval(RgX_coeff(B,5-j), p);
  for(i = 2+j; i <= 6; i++)
...
theta_3_zi(GEN B)
{
  int64_t v2 = myval_zi(RgX_coeff(B,2));
  int64_t v1 = myval_zi(RgX_coeff(B,1));
  int64_t v0 = myval_zi(RgX_coeff(B,0));
...
theta_3_zi2(GEN B)
{
  int64_t v2 = myval_zi2(RgX_coeff(B,2));
  int64_t v1 = myval_zi2(RgX_coeff(B,1));
  int64_t v0 = myval_zi2(RgX_coeff(B,0));
  return min3(6*v2, 3*v1, 2*v0);
}
...
static GEN
factmz(GEN Q, GEN p, int64_t *maxord)
{
  GEN z = FpX_factor_squarefree(Q, p);
  int64_t m = lg(z)-1; /* maximal multiplicity */
  *maxord = m;
...
polymini(GEN H, GEN p)
{
  ...
  int64_t t60, alpha, lambda, maxord, quad = 0, beta = 0;
...
{
  static const int64_t d[9] = { 0,60,30,30,20,15,12,10 }; /* 120 / deg(X) */
  GEN j2 = I->j2, i4 = I->i4, j4 = I->j4, j6 = I->j6, j8 = I->j8;
  GEN val, J, v, Ieps, j10 = I->j10, i12 = I->i12;
  int64_t s, r1, r2, r3, r4, i, eps;
...
static GEN
groupH(int64_t n) { return odd(n)? cyclic(4): dicyclic(2,2); }

static int64_t
get_red(struct red *S, struct igusa_p *Ip, GEN polh, GEN p, int64_t alpha, int64_t r)
{
  GEN val = Ip->val;
  int64_t indice;
  switch(r)
...
static void
ssQ_red(int64_t a, int64_t b, int64_t *n, int64_t *d)
{
  int64_t g = ugcd(labs(a), b);
  if (g > 1) { a /= g; b /= g; }
  *n = a; *d = b;
}
/* denom(a/b); assume b > 0 */
static int64_t
ssQ_denom(int64_t a, int64_t b)
{
  int64_t g = ugcd(labs(a), b);
  return g == 1? b: b / g;
}
...
static void
get_nr(int64_t d, int64_t a, int64_t b, int64_t *n, int64_t *r)
{
  int64_t c, A, B;
  ssQ_red(a, b, &A,&B);
  c = d / ugcd(d, B);
 ...
 * assume b > 0 and d > 0 */
static void
get_nrq(int64_t a, int64_t b, int64_t c, int64_t d, int64_t *n, int64_t *r, int64_t *q)
{
  int64_t g, A, B, C, D;
  ssQ_red(a, b, &A,&B);
  ssQ_red(c, d, &C,&D);
...
/* (4.2) */
static int64_t
tame_234_init(struct igusa *I, struct igusa_p *Ip, int64_t *n, int64_t *q, int64_t *r)
{
  int64_t va0, va5, vb2, v12 = -1, flc = 1;
  GEN p = Ip->p;
  switch(Ip->tt)
  {
...
tame_2(struct igusa *I, struct igusa_p *Ip)
{
  ...
  switch(n)
  {
    ...
    case 3: condp = 3;
      ...
      {
        case 1:
          Ip->type = stack_sprintf("[II{%lld}-IV] page 175", (d-2)/3);
          break;
        case 2:
          Ip->type = stack_sprintf("[II{%lld}-IV*] page 175", (d-1)/3);
          break;
    ...
    case 4:
      switch(r)
      {
        case 1:
          ...
          {
            case 1: condp = 3;
              Ip->type = stack_sprintf("[II{%lld}-III] page 177",(d-2)/4);
              Ip->neron = cyclic(d/2); break;
            case 3: condp = 4;
              Ip->type = stack_sprintf("[II*{%lld}-III*] page 178",(d-2)/4);
...
{
  int64_t condp = -1, n, q, r, va5, d1, d2;
  int64_t flc = tame_234_init(I, Ip, &n, &q, &r);
  GEN val = Ip->val;
...
{
  int64_t condp = -1, d1,d2,d3, f1,f2, g, h, n, q, r, vl,vn,vm, e1,e2,e3;
  GEN val = Ip->val;
  (void)tame_234_init(I, Ip, &n, &q, &r);
  vl = val[6]-5*val[1];
  vn = val[7]-6*val[1];
...
/* p = 3 */
static void
tame_567_init_3(struct igusa_p *Ip, int64_t dk,
                int64_t *pd, int64_t *pn, int64_t *pdm, int64_t *pr)
{
  int64_t n = 1 + Ip->r1/6;
...
/* (4.3) */
static void
tame_567_init(struct igusa *I, struct igusa_p *Ip, int64_t dk,
              int64_t *pd, int64_t *pn, int64_t *pdm, int64_t *pr)
{
  int64_t ndk, ddk;
  GEN p = Ip->p, val = Ip->val;
  ...
  if (! odd(val[8]))
  {
    int64_t va0 = myval(I->a0,p), va2 = myval(I->A2,p), va3 = myval(I->A3,p);
    int64_t va5 = myval(I->A5,p), vb2 = myval(I->B2,p);
    int64_t v1 = 2*va3-4*va0-val[1],   v2 = 6*va5-20*va0-5*val[1];
    int64_t v3 = 3*vb2-2*va0-2*val[1], v4 = 10*vb2-2*va5-5*val[1];
    if (v3 >= 0 && v2 >= 0 && v1 >= 0)
    {
      if (v1==0 || v2==0) get_nr(ddk, va0+val[1], 6,pn,pr); /* Prop 4.3.1 (a) */
      else
      { /* Prop 4.3.1 (d) */
        int64_t v5 = myval(subii(mulii(I->A2,I->A3),mului(3,I->A5)),p);
      ...
      case 12:
        ...
        switch(dm)
        {
          case 1:
            switch(r)
            {
              case 3: case 10:
                ...
                Ip->neron = cyclic(2); break;
              case 4: case 9:
                Ip->type = stack_sprintf("[III*-IV-%lld] page 167",(d-13)/12);
          ...
          case 5:
            switch(r)
            {
              case 2: case 3:
                ...
                Ip->neron = cyclic(2); break;
              case 8: case 9:
                Ip->type = stack_sprintf("[III*-IV*-%lld] page 168",(d-17)/12);
                Ip->neron = cyclic(6); break;
          ...
          case 7:
            ...
            {
              case 3: case 4:
                Ip->type = stack_sprintf("[III-IV-%lld] page 167",(d-7)/12);
                Ip->neron = cyclic(6); break;
              case 9: case 10:
          ...
          case 11:
            ...
            {
              case 3: case 8:
                Ip->type = stack_sprintf("[III-IV*-%lld] page 168",(d-11)/12);
                Ip->neron = cyclic(6); break;
              case 2: case 9:
...
{
  int64_t condp = -1, d, d1, n, dm, r, dk;
  GEN val = Ip->val;
  ...
  {
    case 1: condp = 1;
      Ip->type = stack_sprintf("[I{0}-I{%lld}-%lld] page 170",d1,d);
    ...
    case 3: condp = 3;
      ...
      {
        case 1:
          Ip->type = stack_sprintf("[I{%lld}-IV-%lld] page 173",d1/3,(d-1)/3);
          break;
        case 2:
          Ip->type = stack_sprintf("[I{%lld}-IV*-%lld] page 173",d1/3,(d-2)/3);
          break;
        ...
        case 1:
          switch(r)
          {
            case 0: case 1: condp = 3;
              Ip->type=stack_sprintf("[I{%lld}-III-%lld] page 176",d1/4,(d-1)/4);
              Ip->neron = dicyclic(2,d1/4); break;
            case 2: case 3: condp = 4;
              Ip->type=stack_sprintf("[I*{%lld}-III*-%lld] page 177",d1/4,(d-5)/4);
        ...
        case 3:
          switch(r)
          {
            case 0: case 3: condp = 3;
              Ip->type=stack_sprintf("[I{%lld}-III*-%lld] page 176",d1/4,(d-3)/4);
              Ip->neron = dicyclic(2,d1/4); break;
            case 1: case 2: condp = 4;
              Ip->type=stack_sprintf("[I*{%lld}-III-%lld] page 177",d1/4,(d-3)/4);
              Ip->neron = shallowconcat(groupH(d1/4), cyclic(2)); break;
    ...
    case 6:
      switch(dm)
      {
        case 1:
          ...
          {
            case 0: case 1: condp = 3;
              Ip->type = stack_sprintf("[I{%lld}-II-%lld] page 172",d1/6,(d-1)/6);
              Ip->neron = cyclic(d1/6); break;
            case 3: case 4: condp = 4;
              Ip->type=stack_sprintf("[I*{%lld}-IV*-%lld] page 174",d1/6,(d-7)/6);
              Ip->neron = shallowconcat(groupH(d1/6), cyclic(3)); break;
            default: pari_err_BUG("tame6 [bug49]");
          }
          break;
        case 2: condp = 4;
          Ip->type = stack_sprintf("[I*{%lld}-II*-%lld] page 174",d1/6,(d-8)/6);
          Ip->neron = groupH(d1/6); break;
        case 4: condp = 4;
          Ip->type = stack_sprintf("[I*{%lld}-II-%lld] page 173",d1/6,(d-4)/6);
          Ip->neron = groupH(d1/6); break;
        case 5:
          switch(r)
          {
            case 0: case 5: condp = 3;
              Ip->type=stack_sprintf("[I{%lld}-II*-%lld] page 172",d1/6,(d-5)/6);
              Ip->neron = cyclic(d1/6); break;
            case 2: case 3: condp = 4;
              Ip->type=stack_sprintf("[I*{%lld}-IV-%lld] page 174",d1/6,(d-5)/6);
              Ip->neron = shallowconcat(groupH(d1/6), cyclic(3)); break;
...
{
  int64_t condp = -1, d, D, d1, d2, n, dm, r, dk;
  GEN val = Ip->val;
...
static int64_t labelm3(GEN polh, int64_t t60, int64_t alpha, int64_t Dmin, struct igusa *I, struct igusa_p *Ip);
static int64_t
tame(GEN polh, int64_t t60, int64_t alpha, int64_t Dmin, struct igusa *I, struct igusa_p *Ip)
{
  int64_t d;
  Ip->tame = 1;
  switch(Ip->tt)
...
/* p = 3 */
static int64_t
quartic(GEN polh, int64_t alpha, int64_t Dmin, struct igusa_p *Ip)
{
  GEN val = Ip->val, p = Ip->p;
  GEN polf = polymini_zi2(ZX_Z_mul(polh, powiu(p, alpha)));
  int64_t condp = -1, d, R, r1, beta;
  r1 = polf[1];
  beta = polf[2];
  R = beta/2;
  switch(Ip->tt)
...
static int64_t
litredtp(int64_t alpha, int64_t alpha1, int64_t t60, int64_t t60_1, GEN polh, GEN polh1,
         int64_t Dmin, int64_t R, struct igusa *I, struct igusa_p *Ip)
{
  GEN val = Ip->val, p = Ip->p;
  int64_t condp = -1, indice, d;
  ...
  { /* (r1,r2) = (0,0), (0,6), (6,0) or (6,6) */
    ...
    Ip->type = stack_sprintf("[I{%lld}-I*{%lld}-%lld] page %lld",
                             indice,d-indice,R, (Ip->tt==6)? 170LL: 180LL);
    return condp;
  }
  if (Ip->tt == 7) pari_err_BUG("litredtp [switch ri]");
  {
    struct red __S1, __S2, *S1 = &__S1, *S2 = &__S2;
    int64_t f1 = get_red(S1, Ip, polh1, p, alpha1, Ip->r1);
    int64_t f2 = get_red(S2, Ip, polh,  p, alpha,  Ip->r2);
    /* reorder to normalize representation */
    if (S1->tnum > S2->tnum || (S1->tnum == S2->tnum && f1 > f2))
    { struct red *S = S1; S1 = S2; S2 = S; }
    Ip->type = stack_sprintf("[%s-%s-%lld] pages %s", S1->t,S2->t, R, S1->pages);
...
static int64_t
labelm3(GEN h1, int64_t t60_1, int64_t alpha1, int64_t Dmin, struct igusa *I, struct igusa_p *Ip)
{
  GEN h, pm, vs, val = Ip->val, p = Ip->p;
  int64_t alpha, t60, lambda, beta, R;
...
/* p = 3 */
static int64_t
quadratic(GEN polh, int64_t alpha, int64_t Dmin, struct igusa *I, struct igusa_p *Ip)
{
  int64_t alpha1 = alpha, beta, t6, R;
...
genus2localred(struct igusa *I, struct igusa_p *Ip, GEN p, GEN polmini)
{
  ...
  int64_t i, vb5, vb6, d, Dmin, alpha, lambda, t60;
  int64_t condp = -1, indice, vc6, mm, nb, dism;
...
genus2red(GEN PQ, GEN p)
{
  ...
  for (i = 1; i < l; i++)
  {
    ...
    struct igusa_p Ip;
    int64_t f = genus2localred(&I, &Ip, q, gel(vecmini,i));
